...
describe('justEcho("hi")', () => {
    it('should be equal to ["hi"]', (done) => {
        ...
        return done(new chai.AssertionError(`Expected Array.isArray(${JSON.stringify(ret)}) to be true`));
    });
});

describe('shelljs.exec("npm run --silent cli -- hi").stdout', () => {
    it('should be equal to "hi\\n"', (done) => {
        const ret = shelljs.exec('npm run --silent cli -- hi').stdout;
